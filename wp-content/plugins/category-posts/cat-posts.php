...
function load_textdomain() {
  load_plugin_textdomain( 'cat-posts', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' ); 
}
...
class Widget extends \WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'cat-post-widget', 'description' => __('List single category posts','cat-posts'));
		parent::__construct(WIDGET_BASE_ID, __('Category Posts','cat-posts'), $widget_ops);
	}
...
?>    
        <h4 data-panel="title"><?php _e('Title','cat-posts')?></h4>
        <div>
            <p>
                <label for="<?php echo $this->get_field_id("title"); ?>">
                    <?php _e( 'Title','cat-posts' ); ?>:
            ...
            <p>
                <label for="<?php echo $this->get_field_id("title_link"); ?>">
                    ...
                    <?php _e( 'Make widget title link','cat-posts' ); ?>
                </label>
            </p>
            <p>
...
?>
        <h4 data-panel="filter"><?php _e('Filter','cat-posts');?></h4>
        <div>
            <p>
                <label>
                    <?php _e( 'Category','cat-posts' ); ?>:
                    <?php wp_dropdown_categories( array( 'hide_empty'=> 0, 'name' => $this->get_field_name("cat"), 'selected' => $instance["cat"] ) ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("num"); ?>">
                    <?php _e('Number of posts to show','cat-posts'); ?>:
            ...
            <p>
                <label for="<?php echo $this->get_field_id("sort_by"); ?>">
                    <?php _e('Sort by','cat-posts'); ?>:
                    <select id="<?php echo $this->get_field_id("sort_by"); ?>" name="<?php echo $this->get_field_name("sort_by"); ?>">
                        <option value="date"<?php selected( $instance["sort_by"], "date" ); ?>><?php _e('Date','cat-posts')?></option>
                        <option value="title"<?php selected( $instance["sort_by"], "title" ); ?>><?php _e('Title','cat-posts')?></option>
                        <option value="comment_count"<?php selected( $instance["sort_by"], "comment_count" ); ?>><?php _e('Number of comments','cat-posts')?></option>
                        <option value="rand"<?php selected( $instance["sort_by"], "rand" ); ?>><?php _e('Random','cat-posts')?></option>
                    </select>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("asc_sort_order"); ?>">
                    <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("asc_sort_order"); ?>" name="<?php echo $this->get_field_name("asc_sort_order"); ?>" <?php checked( (bool) $instance["asc_sort_order"], true ); ?> />
                            <?php _e( 'Reverse sort order (ascending)','cat-posts' ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("exclude_current_post"); ?>">
                    ...
                    <?php _e( 'Exclude current post','cat-posts' ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("hideNoThumb"); ?>">
                    ...
                    <?php _e( 'Exclude posts which have no thumbnail','cat-posts' ); ?>
                </label>
            </p>					
        </div>			
<?php
...
?>        
        <h4 data-panel="thumbnail"><?php _e('Thumbnails','cat-posts')?></h4>
        <div>
            <p>
                <label for="<?php echo $this->get_field_id("thumb"); ?>">
                    <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("thumb"); ?>" name="<?php echo $this->get_field_name("thumb"); ?>"<?php checked( (bool) $instance["thumb"], true ); ?> />
                    <?php _e( 'Show post thumbnail','cat-posts' ); ?>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("thumbTop"); ?>">
                    ...
                    <?php _e( 'Show thumbnails above text','cat-posts' ); ?>
                </label>
            </p>
            <p>
                <label>
                    <?php _e('Thumbnail dimensions (in pixels)','cat-posts'); ?>:<br />
                    <label for="<?php echo $this->get_field_id("thumb_w"); ?>">
                        <?php _e('Width:','cat-posts')?> <input class="widefat" style="width:30%;" type="number" min="1" id="<?php echo $this->get_field_id("thumb_w"); ?>" name="<?php echo $this->get_field_name("thumb_w"); ?>" value="<?php echo esc_attr($instance["thumb_w"]); ?>" />
                    </label>
                    
                    <label for="<?php echo $this->get_field_id("thumb_h"); ?>">
                        <?php _e('Height:','cat-posts')?> <input class="widefat" style="width:30%;" type="number" min="1" id="<?php echo $this->get_field_id("thumb_h"); ?>" name="<?php echo $this->get_field_name("thumb_h"); ?>" value="<?php echo esc_attr($instance["thumb_h"]); ?>" />
                    </label>
                </label>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id("use_css_cropping"); ?>">
                    ...
                    <?php _e( 'CSS crop to requested size ','cat-posts' ); ?>
                </label>
            </p>					
            <p>
                <label for="<?php echo $this->get_field_id("thumb_hover"); ?>">
                    <?php _e( 'Animation on mouse hover:','cat-posts' ); ?>
                </label>
                <select id="<?php echo $this->get_field_id("thumb_hover"); ?>" name="<?php echo $this->get_field_name("thumb_hover"); ?>">
                    <option value="none" <?php selected($thumb_hover, 'none')?>><?php _e( 'None', 'cat-posts' ); ?></option>
                    <option value="dark" <?php selected($thumb_hover, 'dark')?>><?php _e( 'Darker', 'cat-posts' ); ?></option>
                    <option value="white" <?php selected($thumb_hover, 'white')?>><?php _e( 'Brighter', 'cat-posts' ); ?></option>
                    <option value="scale" <?php selected($thumb_hover, 'scale')?>><?php _e( 'Zoom in', 'cat-posts' ); ?></option>
					<option value="blur" <?php selected($thumb_hover, 'blur')?>><?php _e( 'Blur', 'cat-posts' ); ?></option>
                </select>
            </p>
        </div>
<?php
	...
	function form($instance) {
        ...
        ?>
			<h4 data-panel="details"><?php _e('Post details','cat-posts')?></h4>
			<div>
				<p>
					<label for="<?php echo $this->get_field_id("hide_post_titles"); ?>">
						...
						<?php _e( 'Hide post titles','cat-posts' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("excerpt"); ?>">
						<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("excerpt"); ?>" name="<?php echo $this->get_field_name("excerpt"); ?>"<?php checked( (bool) $instance["excerpt"], true ); ?> />
						<?php _e( 'Show post excerpt','cat-posts' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("excerpt_length"); ?>">
						<?php _e( 'Excerpt length (in words):','cat-posts' ); ?>
					</label>
				...
				<p>
					<label for="<?php echo $this->get_field_id("excerpt_more_text"); ?>">
						<?php _e( 'Excerpt \'more\' text:','cat-posts' ); ?>
					</label>
					<input class="widefat" style="width:50%;" placeholder="<?php _e('... more','cat-posts')?>" id="<?php echo $this->get_field_id("excerpt_more_text"); ?>" name="<?php echo $this->get_field_name("excerpt_more_text"); ?>" type="text" value="<?php echo esc_attr($instance["excerpt_more_text"]); ?>" />
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("date"); ?>">
						<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("date"); ?>" name="<?php echo $this->get_field_name("date"); ?>"<?php checked( (bool) $instance["date"], true ); ?> />
						<?php _e( 'Show post date','cat-posts' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("date_format"); ?>">
						<?php _e( 'Date format:','cat-posts' ); ?>
					</label>
				...
				<p>
					<label for="<?php echo $this->get_field_id("date_link"); ?>">
						...
						<?php _e( 'Make widget date link','cat-posts' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("comment_num"); ?>">
						<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("comment_num"); ?>" name="<?php echo $this->get_field_name("comment_num"); ?>"<?php checked( (bool) $instance["comment_num"], true ); ?> />
						<?php _e( 'Show number of comments','cat-posts' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("author"); ?>">
						...
						<?php _e( 'Show post author','cat-posts' ); ?>
					</label>
				</p>
			</div>
			<h4 data-panel="general"><?php _e('General','cat-posts')?></h4>
			<div>
				<p>
					<label for="<?php echo $this->get_field_id("disable_css"); ?>">
						...
						<?php _e( 'Disable the built-in CSS for this widget','cat-posts' ); ?>
					</label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id("hide_if_empty"); ?>">
						...
						<?php _e( 'Hide widget if there are no matching posts','cat-posts' ); ?>
					</label>
				</p>
				<p>
 					<label for="<?php echo $this->get_field_id("hide_social_buttons"); ?>">
 						...
 						<?php _e( 'Hide social buttons in widget output','cat-posts' ); ?>
 					</label>
 				</p>
			</div>
			<h4 data-panel="footer"><?php _e('Footer','cat-posts')?></h4>
			<div>
				<p>
					<label for="<?php echo $this->get_field_id("footer_link"); ?>">
						<?php _e( 'Footer link text','cat-posts' ); ?>:
						<input class="widefat" style="width:60%;" placeholder="<?php _e('... more by this topic','cat-posts')?>" id="<?php echo $this->get_field_id("footer_link"); ?>" name="<?php echo $this->get_field_name("footer_link"); ?>" type="text" value="<?php echo esc_attr($instance["footer_link"]); ?>" />
					</label>
				</p>
			</div>
			<h4 data-panel="mysettings"><?php _e('My settings','cat-posts')?></h4>
			<div class="categoryposts-mysettings-panel-auto-close-panels">
				<p>
					<label for="<?php echo $this->get_field_id("auto_close_panels"); ?>">
						...
						<?php _e( 'Auto close panels','cat-posts' ); ?>
					</label>
				</p>
			</div>
...
function add_action_links ( $links ) {
    $pro_link = array(
        '<a target="_blank" href="http://tiptoppress.com/term-and-category-based-posts-widget/?utm_source=widget_cpw&utm_campaign=get_pro_cpw&utm_medium=action_link">'.__('Get the Pro version','cat-posts').'</a>',
    );
...
function customize_register($wp_customize) {
    ...
    if (count($posts) > 0) {
        $wp_customize->add_section( __NAMESPACE__, array(
            'title'           => __( 'Category Posts Shortcode', 'cat-posts' ),
            'priority'        => 200
        ) );
