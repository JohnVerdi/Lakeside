...
<div class="st-create">
    ...
    <?php
    if(STUser_f::check_lever_partner( $role )){
        ?>
        <a class="btn btn-default pull-right" href="<?php echo esc_url( add_query_arg( 'sc' , 'update-info-partner' , get_permalink() ) ) ?>">
            <?php
            _e("Upgrade Info Partner",ST_TEXTDOMAIN);
            ?>
        </a>
    <?php
    }else{
        ?>
        <a class="btn btn-default pull-right" href="<?php echo esc_url( add_query_arg( 'sc' , 'certificate' , get_permalink() ) ) ?>">
            <?php _e("Upgrade Partner",ST_TEXTDOMAIN); ?>
        </a>
    <?php
    }
    ?>
