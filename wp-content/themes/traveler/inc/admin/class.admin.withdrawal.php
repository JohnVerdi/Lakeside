...
if(!class_exists('STAdminWithdrawal'))
{
    class STAdminWithdrawal extends STAdmin
    {
        ...
        {
            parent::init();
            add_action( 'after_setup_theme', array (__CLASS__, '_check_table_st_withdrawal') );

            add_action('admin_menu', array($this,'st_users_partner_withdrawal_menu'));
            ...
            add_action( 'wp_ajax_st_change_status_withdrawal' , array( $this , 'st_change_status_withdrawal_func' ) );
            add_action( 'wp_ajax_nopriv_st_change_status_withdrawal' , array( $this , 'st_change_status_withdrawal_func' ) );
        ...
        function st_users_partner_withdrawal_menu() {
            if(current_user_can('manage_options')) {
                add_submenu_page('st-users-partner-static-menu', __('Partner Withdrawal', ST_TEXTDOMAIN), __('Partner Withdrawal', ST_TEXTDOMAIN), 'manage_options', 'st-users-partner-withdrawal-menu', array($this, 'st_callback_user_partner_withdrawal_function'));
                /* add_users_page( 'Partner Withdrawal' , 'Partner Withdrawal' , 'read' , 'st-users-partner-withdrawal-menu' , array(
                     $this ,
                     'st_callback_user_partner_withdrawal_function'
                 ) );*/
            }
        }
        ...
        static function _insert($user_id,$data){
            if(!empty($data) and !empty($user_id)){
                ...
                //if(1==1){
                    ...
                    return array('status'=>false,'msg'=>__('You have done withdrawal request  of the current month already. Please wait!',ST_TEXTDOMAIN));
                }
            }else{
                return array('status'=>false,'msg'=>__("Invalid data!",ST_TEXTDOMAIN));
            }
        }
        ...
        function st_change_status_withdrawal_func(){
            ...
            if(!empty($st_withdrawal_id) and !empty($st_status)){
                global $wpdb;
                $wpdb->update( $wpdb->prefix."st_withdrawal", array( 'status' => $st_status , 'message' => $st_message  ), array( 'ID' => $st_withdrawal_id ) );
