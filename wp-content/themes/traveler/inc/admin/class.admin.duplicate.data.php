...
if(!class_exists('STDuplicateData')){
  	class STDuplicateData extends STAdmin{
	    ...
	    public function stDeleteTable(){
	      	$post_types = array('st_hotel','hotel_room', 'st_rental', 'st_cars', 'st_tours', 'st_activity');
	    ...
	    **/
	    ...
	    public function runUpdate( $post_type = 'st_hotel' ){
	    	global $wpdb;
	    	$posts_per_page = 10;
	    	if( $post_type == 'st_hotel' ){
	    		...
	    		$update_meta = get_option('update_meta_hotel', '');
	    		...
	    		if( !$update_meta ){
					...
					update_option( 'update_meta_hotel', 'updated' );
	            ...
	            self::_stSaveData($post_type, $fields, $list_value);
            ...
            if( $post_type == 'hotel_room' ){
                $sql = "SELECT count(ID) FROM {$wpdb->prefix}posts WHERE post_type = '{$post_type}' AND post_status IN ('publish', 'private')";
                $total = (int) $wpdb->get_var( $sql );
	    ...
	    public function _stSaveData($post_type = '', $fields = array(), $data = array()){
	    ...
	    public function _stGetStringInsert($fields, $key, $data){
	        $string = array();
	        $string[0] = "'".$key."'";
	    ...
	    public function _getKeyArray($key, $array){
