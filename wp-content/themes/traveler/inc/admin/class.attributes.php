...
if (!class_exists('STAttribute')) {
    ...
    class STAttribute extends STAdmin
    {
        ...
        {
            self::$inst = &$this;
            add_action('admin_init', array( $this, 'copy_option_from_default') );
        ...
        public function copy_option_from_default(){
            $option = get_option('st_attribute_taxonomy', array());
            if( TravelHelper::is_wpml() ){
                $langs = icl_get_languages('skip_missing=N&orderby=KEY&order=DIR&link_empty_to=str');
                if( !empty( $langs ) ){
                    foreach( $langs as $lang => $data ){
                        $old_option = get_option($this->option_holder_name . '_' . $lang, array() );
                        if( empty( $old_option ) ){
                    ...
                    if( !empty( $langs ) ){
                        foreach( $langs as $lang => $data ){
                            if( $lang != ICL_LANGUAGE_CODE ){
                                $options = get_option( $this->option_holder_name . '_' . $lang, array());
                                unset( $options[$tax] );
        ...
        function get_attributes()
        {
            ...
            return get_option( $this->get_key_by_language(), array());
        }
        ...
        function content()
        {
            ...
            return $this->load_view('attributes/index', false, array( 'class_tax' => $this));
