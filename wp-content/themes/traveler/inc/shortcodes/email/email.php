...
if (!function_exists('st_check_in_out_value')) {
    function st_check_in_out_value(){
        ...
        if($order_id){
            ...
            if ($post_type =='st_hotel' or $post_type =='hotel_room' or $post_type =='hotel_room' or $post_type =='st_rental' or $post_type =='st_cars'){
                $day = '';
        ...
        if($order_id){
            ...
            if(is_array($equipment) && count($equipment)):
                ...
                if(!empty($data['tag'])){
                   ...
                   //$html .= '<tr><td style="padding: 10px; font-weight: bold;"><span style="text-align: left; width: 48%; display: inline-block; padding-left: 25px;">'.$data['title']."</span></td></tr>";
...
if(!function_exists( 'st_email_booking_adult_info' )) {
    function st_email_booking_adult_info($atts = array()){
        ...
        if($order_id){
            $post_id = trim(get_post_meta($order_id, 'item_id', true));
            $post_type = get_post_type($post_id );
            if($post_type == 'st_tours' || $post_type == 'st_activity' || $post_type =="st_hotel"){
                $data = shortcode_atts(array(
                    'title' => 'No. Adults',
                ), $atts);
                ...
                if($post_type =="st_hotel") $adult_price_html ="";
                $html = '
                <table width="100%">
                    <tr>
...
if(!function_exists( 'st_email_booking_item_address' )) {
    function st_email_booking_item_address(){
        ...
        if($order_id){
            ...
            if($post_type == 'st_activity'){
                $address = get_post_meta($item_id,'contact_address',true);
            }
            if(!empty($address)){
...
if(!function_exists('st_default_send_user_new_request_withdrawal')){
    function st_default_send_user_new_request_withdrawal(){
        ...
        return '
            ...
            <tr>
            <td>
                 You can cancel this request any time before [st_last_date_payout_this_month] here :
                  <br>
            </td>
             </tr>
             <tr>
