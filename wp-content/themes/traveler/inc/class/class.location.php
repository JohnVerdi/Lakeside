        ...
        **/
        ...
        static function get_info_by_post_type($location_id=false, $post_type = null)
        {
            ...
            if(TravelHelper::checkTableDuplicate($post_type)){
                ...
                $query="SELECT
                            sum(count_comment) as count_comment,
                            count(post_id) as post_id
                        FROM
                            (
                                SELECT
        ...
        public static function get_min_max_price_location($post_type, $location_id)
        {
            ...
            if ($post_type == 'st_rental') { $location_text = 'location_id';}
            $return = array('price_min'=> 0 , 'price_max'=> 500 , 'detail'=> array('item_has_min_price'=>1));
            $is_st_table = true;
            // for hotel
            if ($post_type =='st_hotel'){
