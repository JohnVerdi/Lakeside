...
if(!class_exists('STWithdrawal'))
{
    class STWithdrawal extends TravelerObject
    {
        ...
        function init()
        {
            ...
            add_action( 'init' , array( $this , 'st_partner_withdrawal' ) , 50 );

            add_action( 'wp_ajax_st_load_more_list_withdrawal' , array( $this , 'get_list_withdrawal' ) );
            add_action( 'wp_ajax_nopriv_st_load_more_list_withdrawal' , array( $this , 'get_list_withdrawal' ) );

            add_action( 'wp_ajax_st_remove_withdrawal' , array( $this , 'st_remove_withdrawal_func' ) );
            add_action( 'wp_ajax_nopriv_st_remove_withdrawal' , array( $this , 'st_remove_withdrawal_func' ) );
        ...
        function st_remove_withdrawal_func(){
            ...
            if(!empty($data_user_id) and !empty($data_date_create)){
                global $wpdb;
                $wpdb->delete( $wpdb->prefix.'st_withdrawal' , array( 'user_id' => $data_user_id , "created"=>$data_date_create ) );
                echo json_encode(
                  array(
                      'status'=>'true',
                      'msg'=>'Xóa thành công !',
